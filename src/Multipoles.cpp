    ...
    while(getline(inFile, words))
    {
      if(words == " Induced Dipole Moments (Debyes) :")
      {
        ...
        for (int i=0;i<Natoms;i++)
        {
          ...
          cout << "IDip read from file x: " << QMMMData[i].MP[bead].IDipx << '\n';
          cout.flush();
          line >> QMMMData[i].MP[bead].IDipy;
          cout << "IDip read from file y: " << QMMMData[i].MP[bead].IDipy << '\n';
          cout.flush();
...
void ExtractGlobalPoles(int& argc, char**& argv)
{
  ...
  stringstream call; //Stream for system calls and reading/writing files
  call.str("");
  call << "LICHEM_Multipoles" << ".err";
  errFile.open(call.str().c_str(),ios_base::out);
  call.str("");
  call << "LICHEM_Multipoles" << ".log";
  logFile.open(call.str().c_str(),ios_base::out);
  logFile << fixed;
  ...
  logFile << '\n';
  logFile << "Global frame multipole moments (a.u.):";
  logFile << '\n' << '\n';
  for (int i=0;i<Natoms;i++)
  {
    logFile << "Atom " << i << ": ";
    logFile << QMMMData[i].MMTyp << '\n';
    logFile << "Position (x,y,z):" << '\n';
    logFile << " "; //Sometimes you just need a little space
    logFile << QMMMData[i].P[bead].x << " ";
    logFile << QMMMData[i].P[bead].y << " ";
    logFile << QMMMData[i].P[bead].z << '\n';
    logFile << "Charge and dipole (x,y,z):" << '\n';
    logFile << " "; //Sometimes you just need a little space
    logFile << QMMMData[i].MP[bead].q << " ";
    logFile << QMMMData[i].MP[bead].Dx << " ";
    logFile << QMMMData[i].MP[bead].Dy << " ";
    logFile << QMMMData[i].MP[bead].Dz << '\n';
    logFile << "Quadrupole tensor (x,y,z):" << '\n';
    logFile << " "; //Sometimes you just need a little space
    logFile << QMMMData[i].MP[bead].Qxx << " ";
    logFile << QMMMData[i].MP[bead].Qxy << " ";
    logFile << QMMMData[i].MP[bead].Qxz << '\n';
    logFile << " "; //Sometimes you just need a little space
    logFile << QMMMData[i].MP[bead].Qxy << " ";
    logFile << QMMMData[i].MP[bead].Qyy << " ";
    logFile << QMMMData[i].MP[bead].Qyz << '\n';
    logFile << " "; //Sometimes you just need a little space
    logFile << QMMMData[i].MP[bead].Qxz << " ";
    logFile << QMMMData[i].MP[bead].Qyz << " ";
    logFile << QMMMData[i].MP[bead].Qzz << '\n';
    logFile << '\n';
  }
  logFile << "Total charge: " << totChg;
  logFile << '\n' << '\n';
  logFile.close();
  errFile.close();
  //Quit
